...
type Props = NativeStackScreenProps<any, 'Namjari'>;

// ─── Constants ────────────────────────────────────────────────────────────────
const TOTAL_STEPS = 2;
const STEP_LABELS = ['আবেদন তথ্য', 'সংযুক্তি'];
...
interface AttachmentSection {
  ...
  checked: boolean;
  aid: string;             // আইডি field
  remark: string;          // মন্তব্য field
  files: AttachmentEntry[];
  expanded: boolean;       // accordion open/closed
}

const INITIAL_ATTACHMENT_SECTIONS: AttachmentSection[] = [
  { key: 'a1', label: 'মালিকানা দলিল',                                                            checked: false, aid: '', remark: '', files: [], expanded: false },
  { key: 'a2', label: 'উত্তরাধিকারীদের সনাতকরণ/ওয়ারিশান সনদপত্র কিপ',                          checked: false, aid: '', remark: '', files: [], expanded: false },
  { key: 'a3', label: 'সি,এস/এস,এ, পচি/সিটি জরিপ কিপ',                                          checked: false, aid: '', remark: '', files: [], expanded: false },
  { key: 'a4', label: 'ডুপিকেট কার্বন রশিদ/নকল কপি (ডি.সি.আর)',                                  checked: false, aid: '', remark: '', files: [], expanded: false },
  { key: 'a5', label: 'সরকারী প্রটার ধরণের জন্য সংরিস্ট সংষার/মন্ত্রালয় অনুমোদনের প্রমাণপত্র', checked: false, aid: '', remark: '', files: [], expanded: false },
  { key: 'a6', label: 'ল্যান্ড ট্যাক্স/ভূমি কর রিশদ',                                            checked: false, aid: '', remark: '', files: [], expanded: false },
];
...
// ─── Main Screen ──────────────────────────────────────────────────────────────
export function NamjariApplicationScreen({ navigation }: Props) {
  ...
  const [form, setForm] = useState({
    holdingNo: '', zone: '২ মিরপুর', ward: '', sector: '', area: '', road: '',
  ...
  // Step 2 — attachment sections
  const [sections, setSections] = useState<AttachmentSection[]>(INITIAL_ATTACHMENT_SECTIONS);
  ...
  const addFile = (uri: string, name: string, isImage: boolean) => {
    const entry: AttachmentEntry = { id: Date.now().toString(), type: '', uri, name, isImage };
    setSections(prev => prev.map(s =>
  ...
  const pickFromGallery = async () => {
    ...
    if (!result.canceled && result.assets.length > 0) {
      ...
      addFile(a.uri, a.fileName ?? 'image.jpg', true);
    }
  };
  ...
  const pickFromCamera = async () => {
    ...
    if (!result.canceled && result.assets.length > 0) {
      const a = result.assets[0];
      addFile(a.uri, a.fileName ?? 'photo.jpg', true);
    }
  };
  ...
  const validateStep = () => {
    if (step === 1) {
      if (!form.applicationDate || !form.applicantName) {
  ...
  const handleSubmit = () => {
    Alert.alert('সফল', 'আপনার নামজারির আবেদন সফলভাবে জমা দেওয়া হয়েছে।', [
  ...
  // ── Step 2 render — Attachment accordion sections ──────────────────────────
  const renderStep2 = () => (
    <View style={styles.formCard}>
      ...
      {sections.map((section) => (
        <View key={section.key} style={styles.attachSection}>
          ...
          {/* Checkbox row — tap to toggle check + expand */}
          <TouchableOpacity
            style={styles.checkRow}
            onPress={() => toggleChecked(section.key)}
            activeOpacity={0.7}
          >
            ...
            {/* Expand/collapse if already checked */}
            {section.checked && (
          ...
          {section.checked && section.expanded && (
            <View style={styles.attachBody}>
              ...
              <StyledInput
                value={section.aid}
                onChangeText={v => updateSection(section.key, { aid: v })}
              />
              ...
              <StyledInput
                ...
                onChangeText={v => updateSection(section.key, { remark: v })}
                multiline
              />
              ...
              {/* সংযুক্ত করুন button */}
              <TouchableOpacity
                style={styles.attachUploadBtn}
                onPress={() => openPickerFor(section.key)}
                activeOpacity={0.8}
              >
  ...
  return (
    <SafeAreaView style={styles.root} edges={['top']}>
      ...
      <View style={styles.stepWrapper}>
        <Text style={styles.formTitle}>নামজারির আবেদন</Text>
          ...
          <View style={styles.previewFooter}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                setSections(prev => prev.map(s => ({
                  ...s, files: s.files.filter(f => f.id !== entry.id)
                })));
              }}
            >
...
const styles = StyleSheet.create({
  ...
  stepLabel: { fontSize: 9, color: Colors.gray500, textAlign: 'center', maxWidth: 64 },
  ...
  checkbox: {
    ...
    borderColor: Colors.gray400, justifyContent: 'center', alignItems: 'center',
  ...
  attachUploadBtn: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    gap: 6, backgroundColor: Colors.primary, borderRadius: BorderRadius.md,
