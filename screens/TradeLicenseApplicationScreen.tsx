...
// ─── Main Screen ──────────────────────────────────────────────────────────────
export function TradeLicenseApplicationScreen({ navigation, route }: Props) {
  const isOld = route.name === 'OldTradeLicense';
  ...
  const [f, setF] = useState({
    ...
    // Old TL extra
    issuedLicenseNo: '',
    issuedLicenseDate: '20-Apr-2026',
    renewalEffectiveDate: '20-Apr-2026',
  ...
  const validateStep = () => {
    ...
    if (step === 3 && isOld && (!f.issuedLicenseNo || !f.issuedLicenseDate || !f.renewalEffectiveDate)) {
      Alert.alert('ত্রুটি', 'পুরাতন লাইসেন্সের বাধ্যতামূলক তথ্য পূরণ করুন।'); return false;
    }
    return true;
  };
  ...
  const renderStep3 = () => (
    <>
      ...
      <View style={styles.card}>
        ...
        {/* Old TL extra fields — only shown for পুরাতন */}
        {isOld && (
          <>
            <FieldLabel text="ইস্যুকৃত লাইসেন্স নং (পুরাতনের ক্ষেত্রে)" required />
            <StyledInput value={f.issuedLicenseNo} onChangeText={set('issuedLicenseNo')} />

            <FieldLabel text="ইস্যুকৃত লাইসেন্স তারিখ (পুরাতনের ক্ষেত্রে)" required />
            <StyledInput
              value={f.issuedLicenseDate}
              onChangeText={set('issuedLicenseDate')}
              placeholder="dd-Mon-yyyy"
            />

            <FieldLabel text="নবায়ন কার্যকর করার তারিখ (পুরাতনের ক্ষেত্রে)" required />
            <StyledInput
              value={f.renewalEffectiveDate}
              onChangeText={set('renewalEffectiveDate')}
              placeholder="dd-Mon-yyyy"
            />
          </>
        )}
  ...
  return (
    <SafeAreaView style={styles.root} edges={['top', 'bottom']}>
      ...
      <View style={styles.stepWrapper}>
        <Text style={styles.formTitle}>{isOld ? 'পুরাতন' : 'নতুন'} ট্রেড লাইসেন্স এর আবেদন</Text>
