...
const HOLDING_TAX_ITEMS = [
  { id: '1',labelid:'h_apply', label: 'আবেদন',        icon: null,image: require('../assets/icons/1.png'), // your PNG 
  screens:'HoldingTaxAbedon'  },
  { id: '2',labelid:'h_search', label: 'অনুসন্ধান', image: null,     icon: 'file-search-outline' ,screens:'HoldingTaxAnusandhan' },
  { id: '3',labelid:'h_enum', label: 'নাম্বার সংযুক্ত',image: null,     icon: 'calculator-variant-outline' ,screens:'EHoldingNumber' },
  { id: '4',labelid:'h_due', label: 'বকেয়া বিবরণী', image: null,    icon: 'clipboard-list-outline',screens:'BokeyaBiboroni' },
  { id: '5',labelid:'h_pay', label: 'কুইক পে', image: null,        icon: 'timer-outline',screens:'QuickPay' },
    ...
    children: [
      {
        ...
        children: [
          { id: 'h_apply',  label: 'আবেদন' ,screens:'HoldingTaxAbedon' },
          { id: 'h_search', label: 'অনুসন্ধান' ,screens:'HoldingTaxAnusandhan'},
        ],
      },
      { id: 'h_enum',   type: 'item', label: 'ই-হোল্ডিং নাম্বার সংযুক্ত করুন',screens:'EHoldingNumber' },
      { id: 'h_due',    type: 'item', label: 'বকেয়া বিবরণী' ,screens:'BokeyaBiboroni' },
      { id: 'h_quick',  type: 'item', label: 'কুইক পে' ,screens:'QuickPay'},
      { id: 'h_online', type: 'item', label: 'অনলাইন কর পরিশোধের বিবরণ',screens:'OnlinePaymentDetails' },
    ],
  },
  {
...
// function ServiceIcon({ item, onPress }: { item: any; onPress?: () => void }) {
//   return (
//     <TouchableOpacity style={styles.iconItem} activeOpacity={0.7} onPress={onPress}>
  ...
  return (
    <View style={styles.section}>
      ...
      <View style={styles.iconGrid}>
        {items.map((item) => (
          <ServiceIcon key={item.id} item={item} onPress={() => onItemPress?.(item)} />
        ))}
      </View>
    </View>
  );
...
function SidebarAccordion({ item, onChildPress }: { item: any; onChildPress: (child: any) => void }) {
...
function SidebarSection({ section, onItemPress }: { section: any; onItemPress: (item: any) => void }) {
  return (
    <View>
      ...
      {section.children?.map((child: any) => {
        if (child.type === 'accordion') {
          return <SidebarAccordion key={child.id} item={child} onChildPress={onItemPress} />;
        }
        return (
...
export function DashboardScreen({ navigation, route }: Props) {
  ...
  const handleSidebarNav = (item: any) => {
    closeSidebar();
    if (item.screens) {
      setTimeout(() => navigation.navigate(item.screens), 300);
    }
  };
    ...
    <SafeAreaView style={styles.root} edges={['top']}>
      ...
      <ScrollView style={styles.body} showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        <ServiceSection
          ...
          items={HOLDING_TAX_ITEMS}
          onItemPress={(item) => {
            navigation.navigate(item.screens);
            // if (item.labelid === 'h_apply')    navigation.navigate(item.screens);
            // if (item.labelid === 'h_search')   navigation.navigate(item.screens);
            // if (item.labelid === 'h_enum')   navigation.navigate(item.screens);
          }}
        />
        <ServiceSection
      ...
      <Animated.View style={[styles.sidebar, { transform: [{ translateX: sidebarAnim }] }]}>
        <ScrollView showsVerticalScrollIndicator={false} bounces={false}>
          ...
          {SIDEBAR_NAV.map((section) => (
            <SidebarSection key={section.id} section={section} onItemPress={handleSidebarNav} />
          ))}
...
const styles = StyleSheet.create({
  ...
  iconCircle: { width: 64, height: 64, borderRadius: 32, backgroundColor: Colors.gray200, justifyContent: 'center', alignItems: 'center', marginBottom: 6, borderWidth: 1.5, borderColor: Colors.gray200 },
